...
class Player:
    ...
    def get_list_index_action(self, state):
        ...
        self_st = numpy.array(state[6:12])
        self_st_const = numpy.array(state[134:139])
        normal_cards = state[34:124]
        board_stocks = numpy.array(state[:6])

        # Check mua thẻ
        cards_check_buy = [i for i in range(90) if normal_cards[i] in [-1,4]]
        cards_can_buy = []
        ...
        for card_id in cards_can_buy:
            list_index_action.append(card_id)
        
        # Check úp thẻ
        if normal_cards.count(4) < 3:
            cards_can_upside = [i for i in range(90) if normal_cards[i] == -1]
            ...
            if return_stock: # úp thẻ trả nguyên liệu, index action: 180->719
                ...
                for card_id in cards_can_upside:
                    for st_id in st_can_return:
                        list_index_action.append(card_id*6+st_id+180)
                ...
                # Úp thẻ hiện: 90->179
                for card_id in cards_can_upside:
                    list_index_action.append(card_id+90)
    ...
    def get_list_state(self, dict_input):
        ...
        # Lần lượt, stocks của 4 người chơi, của bản thân đầu tiên: 6->29
        players_stocks = []
        players_stocks = list(self.stocks)
        for p in dict_input['Players']:
            players_stocks += list(p.stocks)

        if players_stocks.__len__() < 24:
            for k in range(int((24-players_stocks.__len__())/6)):
                players_stocks += [0,0,0,0,0,0]
        
        # Lần lượt, điểm của 4 người chơi, của bản thân đầu tiên: 30->33
        players_score = [self.score] + [p.score for p in dict_input['Players']]
        while players_score.__len__() < 4:
            players_score.append(0)

        # 34->133
        # 100 phần tử, thẻ thường (0-89), thẻ quý tộc (90-99)
        # Quy ước:
        ...
        # 0->99: thẻ thường và thẻ quý tộc
        ...
        return [int(ii) for ii in (board_stocks + players_stocks + players_score + cards + players_stocks_const + [dict_input['Turn'], dict_input['Players'].__len__() + 1, 1 if dict_input['End_game'] else 0])]